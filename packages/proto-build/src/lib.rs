...
use syn::{
    ...
    parse_quote, Attribute, Fields, File, Ident, Item, ItemMod, Type,
};
use walkdir::WalkDir;
...
pub fn run() {
    ...
    copy_and_transform_generated_files(&temp_osmosis_dir, &proto_dir, query_services);
...
fn compile_osmosis_proto(out_dir: &Path) -> HashMap<String, ServiceDescriptorProto> {
    ...
    let proto_includes_paths = [
        ...
        osmosis_dir.join("third_party/proto"),
    ];
...
fn copy_and_transform_generated_files(
    from_dir: &Path,
    to_dir: &Path,
